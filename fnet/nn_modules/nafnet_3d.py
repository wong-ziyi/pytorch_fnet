...
class NAFNet3D(nn.Module):
    def __init__(self, img_channel=1, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], dw_expand=2, ffn_expand=2):
        super().__init__()
        ...
        for num in dec_blk_nums:
            self.ups.append(
                nn.Sequential(
                    nn.Conv3d(chan, chan * 4, 1, bias=False),  # 8 = 2**3 for 3D and / 2 for upsampling
...
class NAFNetLocal3D(Local_Base, NAFNet3D):
    def __init__(self, *args, train_size=(1, 1, 32, 64, 64), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFNet3D.__init__(self, *args, **kwargs)

        N, C, D, H, W = train_size
        base_size = (int(D * 1.5), int(H * 1.5), int(W * 1.5))
